...
exports.getalladvanceBooking = async (req, res) => {
  try {
      const allAdvanceBooking = await advanceBookingmodel.find({});
       res.status(200).json({ success: true, data: allAdvanceBookings });
  } catch (error) {
      console.log(error);
      res.status(500).json({ success: false, message: 'An error occurred' });
  }
